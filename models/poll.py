...
#                         icon_url=SETTINGS.author_icon)
        ...
        if self.anonymous:
            embed = self.add_field_custom(name='**Anonymous**', value='', embed=embed)
...
########################################################################################################################################
        if not (await self.is_open()):
            ...
            msg = 'VOTES\n'
            for i, o in enumerate(self.options_reaction):
                if not self.options_reaction_default and not self.options_reaction_emoji_only:
                    msg += AZ_EMOJIS[i] + " "
                msg += o + ":"
                c = 0
                for vote in self.full_votes:
                    member = self.server.get_member(int(vote.user_id))
                ...
                msg += '\n\n'
            custom_text = msg
            embed.set_footer(text=custom_text)
########################################################################################################################################
        else:
